 ...
 * The design for using statically defined error codes (rest codes) is due to limitations in Java's matching syntax. In
 * order to provide for clean logic branches for handling various server-side conditions, it works best in Java to use
 * an enum. This allows us to easily use switch / case statements.
...
public enum MantaErrorCode {
    ...
    ENTITY_ALREADY_EXISTS_ERROR("EntityAlreadyExists"),
