...
public class SupportedCipherDetailsTest {
    public void size1024bCalculationWorksRoundTripForAllCiphers() {
        for (SupportedCipherDetails cipherDetails : SupportedCiphersLookupMap.INSTANCE.values()){
            final long size = 1024;
            sizeCalculationWorksRoundTrip(cipherDetails, size);
        }
    }

    public void size0bCalculationWorksRoundTripForAllCiphers() {
        for (SupportedCipherDetails cipherDetails : SupportedCiphersLookupMap.INSTANCE.values()){
            final long size = 0;
            sizeCalculationWorksRoundTrip(cipherDetails, size);
        }
    ...
    public void size2009125bCalculationWorksRoundTripForAllCiphers() {
        for (SupportedCipherDetails cipherDetails : SupportedCiphersLookupMap.INSTANCE.values()){
    ...
    public void ciphertextSizeCalculationWorksForAllCiphers() throws Exception {
        for (SupportedCipherDetails cipherDetails : SupportedCiphersLookupMap.INSTANCE.values()){
            sizeCalculationWorksComparedToActualCipher(cipherDetails);
        }
    }

    private void sizeCalculationWorksRoundTrip(final SupportedCipherDetails cipherDetails,
    ...
    private void sizeCalculationWorksComparedToActualCipher(final SupportedCipherDetails cipherDetails) throws Exception {
