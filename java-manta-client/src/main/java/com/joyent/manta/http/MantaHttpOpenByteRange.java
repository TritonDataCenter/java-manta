...
/**
 * Class describing a manta http object byte range decorator (open).
 *
 ...
 * @since 3.0.0
 */
public class MantaHttpOpenByteRange<T extends NullRange & FixableRange<T> & AdjustableRange<T, U, V>,
    ...
    /**
     * {@code MantaHttpByteRange} constructor.
     *
     * @param range range to add manta http functionality to
     */
    public MantaHttpOpenByteRange(final U range) {
    ...
    public void addTo(final MantaHttpHeaders headers) {
        ...
        if (start > 0) {
            ...
            byteRange = String.valueOf(start) + "-";
    ...
    @Override
    public MantaHttpOpenByteRange<T, U, V> doFix(final long length) {
        ...
        U fixed = range.doFix(length);

        return new MantaHttpOpenByteRange<T, U, V>(fixed);
    }
    ...
    public boolean equals(final Object o) {
        ...
        MantaHttpOpenByteRange<?, ?, ?> mob = (MantaHttpOpenByteRange<?, ?, ?>) o;

        return range.equals(mob.range);
    }
     ...
     */
    private static void validate(final OpenRange range) {
